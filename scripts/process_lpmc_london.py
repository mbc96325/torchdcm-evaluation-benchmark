...
def load_raw(path: Path) -> pd.DataFrame:
    """Load the archived canonical CSV without contacting an external server."""
    if not path.exists():
        raise FileNotFoundError(
            f"Archived LPMC input not found: {path}. "
            "Restore data/raw/lpmc_london from the repository checkout."
        )
...
def main() -> int:
    ...
    parser.add_argument("--zip", action="store_true", help="Create a local processed-data archive.")
